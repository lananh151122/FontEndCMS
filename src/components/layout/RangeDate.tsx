...
const RangeDate: React.FC<any> = ({ rangeDate, setRangeDate }) => {
  ...
  const rangePresets: TimeRangePickerProps['presets'] = [
    ...
    { label: 'Tuần trước', value: [dayjs().startOf('week').subtract(1, 'week'), dayjs().endOf('week').subtract(1, 'week')] },
    { label: 'Tháng này', value: [dayjs().startOf('month'), dayjs().endOf('month')] },
    { label: 'Tháng trước', value: [dayjs().startOf('month').subtract(1, 'month'), dayjs().endOf('month').subtract(1, 'month')] },
    { label: '3 tháng trước', value: [dayjs().startOf('month').subtract(2, 'month'), dayjs().endOf('month').subtract(1, 'month')] },
  ...
  return (
    <Space direction="vertical">
      <RangePicker presets={rangePresets} value={rangeDate} onChange={setRangeDate} />
    </Space>
  );
};
